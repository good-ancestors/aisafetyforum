...
function md5(input: string): string {
  ...
  function convertToWordArray(str: string): number[] {
    ...
    const wordArray: number[] = new Array(wordCount * 16).fill(0);
    let byteCount = 0;
 ...
 * Priority: custom avatarUrl > Gravatar > initials fallback
 */
export default function ProfileAvatar({
  ...
  // If custom avatar URL exists and hasn't errored, use it
  ...
  // Try Gravatar if no custom URL or custom errored
  if (!gravatarError) {
    return (
      // eslint-disable-next-line @next/next/no-img-element
      <img
        src={`https://www.gravatar.com/avatar/${md5(email.toLowerCase().trim())}?s=${pxSize * 2}&d=404`}
        alt={name || 'Profile'}
        className={`${sizeClass} rounded-full object-cover ${className}`}
